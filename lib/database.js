    ...
    try {
        ...
        if (needsMigration) {
            ...
            db.exec(`
                ...
                INSERT INTO domains_new (id, user_id, subdomain, domain, record_type, record_value, cf_record_id, proxied, ttl, status, remark, created_at, updated_at)
                SELECT id, user_id, subdomain, domain, record_type, record_value, cf_record_id, proxied, ttl, status, remark, created_at, updated_at FROM domains;
                DROP TABLE domains;
...
function updateDomainCfRecordId(id, cfRecordId) {
    return db.prepare('UPDATE domains SET cf_record_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(cfRecordId, id);
}
...
function getAllVerifications() {
    return db.prepare('SELECT * FROM dns_verifications').all();
}
