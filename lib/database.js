...
// 域名操作
function createDomain(userId, subdomain, recordType, recordValue, cfRecordId, proxied, ttl, remark) {
    const result = db.prepare(
        `INSERT INTO domains (user_id, subdomain, record_type, record_value, cf_record_id, proxied, ttl, remark) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(userId, subdomain, recordType, recordValue, cfRecordId, proxied ? 1 : 0, ttl, remark || '');
    return result.lastInsertRowid;
}
...
function getDomainBySubdomain(subdomain) {
...
function suspendUserDomains(userId) {
    return db.prepare("UPDATE domains SET status = 'suspended', cf_record_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?").run(userId);
}
...
function countUserDomains(userId) {
    const row = db.prepare('SELECT COUNT(*) as count FROM domains WHERE user_id = ?').get(userId);
    return row.count;
}
