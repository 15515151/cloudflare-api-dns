...
class DNSPodAPI {
    ...
    async listRecords(params = {}) {
        ...
        try {
            ...
            // DNSPod throws error when record list is empty. We catch it and return []
            if (error.message && (error.message.includes('为空') || error.message.includes('没有') || error.code === 'ResourceNotFound.NoDataOfRecord')) {
                return [];
            }
    ...
    async createRecord(type, name, content, proxied = false, ttl = 600) {
        ...
        try {
            ...
            console.error(`[DNSPod Create Error] Req: ${JSON.stringify({ Domain: this.domain, SubDomain: subdomain, RecordType: type, Value: content, TTL: ttl })} Error:`, error);
            throw new Error(`DNSPod API 错误: ${error.message}`);
        }
    }
    ...
    async updateRecord(recordId, fields) {
        ...
        try {
            // Retrieve the specific record info to get its SubDomain and Type
            const recordInfoReq = {
                Domain: this.domain,
                RecordId: parseInt(recordId)
            };
            const recordResp = await this.client.DescribeRecord(recordInfoReq);
            existingRecord = recordResp.RecordInfo;
        } catch (e) {
        ...
        try {
            const req = {
                Domain: this.domain,
                RecordType: existingRecord.RecordType || existingRecord.Type,
                RecordLine: existingRecord.RecordLine || existingRecord.Line || "默认",
                Value: content,
                RecordId: parseInt(recordId),
                SubDomain: existingRecord.SubDomain || existingRecord.Name,
                TTL: ttl
            };
